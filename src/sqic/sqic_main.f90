!     ( Last modified on 28 Feb 2014 at 08:50:00 )
...
!+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  integer(ip),   pointer :: hs(:), hEtype(:), indA(:), locA(:), jcol2(:)
  integer(ip),   pointer :: indH(:), locH(:), irow(:), jcol(:), irow2(:)
  real(rp),      pointer :: bl(:), bu(:), x(:), pi(:), rc(:), cObj(:)
  real(rp),      pointer :: valA(:), valH(:), zero(:), cval(:), cval2(:), b(:)
