! THIS VERSION: CUTEST 1.0 - 09/03/2013 AT 17:00 GMT.
...
!  dummy arguments
      ...
      INTEGER, INTENT( IN ) :: input, io_buffer, out
      INTEGER, INTENT( OUT ) :: status, n, m
      REAL ( KIND = wp ), INTENT( OUT ) :: f
      CHARACTER ( LEN = 10 ), INTENT( OUT ) :: p_name
      REAL ( KIND = wp ), ALLOCATABLE, INTENT( OUT ),                          &
...
!  local variables
      ...
      INTEGER :: alloc_status, i, j, l, liw, la, lh, nea, neh
      CHARACTER( LEN = 1 ) :: a_type, h_type
      LOGICAL :: pert_hess
      INTEGER, ALLOCATABLE, DIMENSION( : ) :: A_tmp, H_tmp, IW
